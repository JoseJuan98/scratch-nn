...
class DeepNN(NeuralNet):
    ...
    def train(self,
              ...
              plot=False):
        """
        Implements an L-layer deep neural network: [LINEAR->RELU]*(L-1)->LINEAR->SIGMOID and computes the
        weights of the neurons.

        Args:
            X (numpy.ndarray): data, numpy array of shape (num_px * num_px * 3, number of examples)
            Y (numpy.ndarray): true "label" vector of shape (1, number of examples)
            ...
            plot (bool): if True, after finishing plots the costs per iteration
